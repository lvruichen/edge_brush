...
#include "edge_brush/groundFilter.h"
...
void GroundPlaneFilter::updateParams() {
  ...
  nh_local_.param<double>("thres_dist", thres_dist_, 0.1);
  nh_local_.param<string>("topic_str", topic_str_, "/rslidar_points_trans");
...
void GroundPlaneFilter::callBack(
  ...
  sensor_msgs::PointCloud2 ros_point_ground;
  pcl::toROSMsg(point_ground_, ros_point_ground);
  ros_point_ground.header.stamp = fused_cloud->header.stamp;
  ros_point_ground.header.frame_id = fused_cloud->header.frame_id;
  point_ground_pub_.publish(ros_point_ground);
}
